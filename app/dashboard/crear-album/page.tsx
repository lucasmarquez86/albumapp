...
const pitchFallback='linear-gradient(180deg,#07182f 0 22%,#123e69 23% 47%,#23603e 48% 56%,#4c9449 57% 100%)'
export default function Page(){
 ...
 const pitchImage=pitch?`url(${pitch})`:pitchFallback
      ...
      <div className="mt-6 grid lg:grid-cols-[310px_1fr] gap-6 items-start">
       <div><p className="text-[10px] font-black mb-2">FIGURITA</p><div className="mx-auto bg-[#f7f7f5] border-[9px] border-white shadow-[0_18px_45px_rgba(0,0,0,.16)] w-full max-w-[300px] aspect-[.70] p-[5px]">
        <div className="relative h-[76%] overflow-hidden bg-[#eee]">
         ...
         <div className="absolute inset-x-0 top-[21%] bottom-0" style={{background:p,opacity:.42}}/>
         <Pattern/>
         <div className="absolute z-10 top-[26%] left-3 text-7xl font-black" style={{color:'#fff',opacity:.16}}>13</div>
        ...
        <div className="h-[24%] bg-[#f5f5f3] grid grid-cols-[30%_1fr_25%] border-t border-black/10">
         <div className="p-2 flex flex-col justify-center gap-2 text-[9px] font-black"><span>18-04-97</span><span>1,84 M</span><span className="border border-black/30 text-center px-1">PANINI</span></div>
         <div className="p-1 flex flex-col justify-center text-center"><span className="text-[9px]">IGNACIO</span><b className="text-xl leading-none">VÁZQUEZ</b><span className="mt-2 py-1 text-[9px] text-white font-black" style={{background:p}}>DEFENSOR</span></div>
         <div className="border-l border-black/10 flex flex-col items-center justify-center"><span className="text-2xl">🇦🇷</span><span className="text-[8px] font-black mt-1">ARGENTINA</span></div>
        </div>
       </div></div>
       <div><p className="text-[10px] font-black mb-2">ÁLBUM · DOBLE PÁGINA</p><div className="relative aspect-[16/9] rounded-xl overflow-hidden shadow-[0_18px_45px_rgba(0,0,0,.16)] bg-cover bg-center" style={{backgroundImage:stadium}}><div className="absolute inset-0" style={{background:p,opacity:.36}}/><Pattern soft/><div className="absolute inset-y-0 left-1/2 w-px bg-black/35 shadow-[0_0_8px_rgba(0,0,0,.4)]"/><div className="relative h-full grid grid-cols-6 grid-rows-2 gap-[3%] p-[5%]">{Array.from({length:12},(_,i)=><div key={i} className="relative bg-white p-[5%] shadow-lg rotate-0"><div className="h-full border border-white/40 bg-black/10 grid place-items-center"><span className="text-white/80 font-black text-xs">{String(i+1).padStart(3,'0')}</span></div></div>)}</div></div><div className="mt-4 bg-white/70 rounded-xl p-4 text-xs leading-relaxed"><b>Regla visual:</b> cancha real + capa del color primario + pattern. Las figuritas siempre llevan marco blanco.</div></div>
      </div>
     </div>
    </section>
   </div>
